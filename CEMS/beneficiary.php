...
if(!in_array($_SESSION['role'], $allowed_roles)){
    ...
    $donor_roles = ['Donor','Donor/Partner'];
    $stu_roles   = ['Student'];
    ...
    elseif(in_array($_SESSION['role'], $donor_roles))  header("Location: donor.php");
...
$allowed_sections = ['profile','location','project','feedback'];
...
<title>CEMS - Beneficiary Portal</title>
...
body{
    min-height:100vh;
    background:linear-gradient(145deg,#020c12 0%,#041824 25%,#061e2e 50%,#041824 75%,#020c12 100%);
...
.sidebar-brand-icon{width:28px;height:28px;border-radius:7px;background:linear-gradient(135deg,var(--cyan),var(--teal));display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.sidebar-brand-sub{font-size:0.55rem;color:rgba(6,182,212,0.7);font-weight:600;}
.sidebar-user{background:rgba(255,255,255,0.05);border:1px solid rgba(6,182,212,0.12);border-radius:10px;padding:0.6rem;margin-bottom:1.2rem;display:flex;flex-direction:column;align-items:center;gap:4px;}
.sidebar-avatar{width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,var(--cyan),var(--teal));display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.95rem;color:white;}
...
@keyframes glow{0%,100%{box-shadow:0 0 0 0 rgba(6,182,212,0.5);}50%{box-shadow:0 0 0 4px rgba(6,182,212,0);}}
...
.nav-item.active{background:rgba(6,182,212,0.18);border-color:rgba(6,182,212,0.35);color:white;font-weight:700;box-shadow:0 0 12px rgba(6,182,212,0.15);}
...
.nav-badge{position:absolute;top:4px;right:4px;width:8px;height:8px;background:#EAB308;border-radius:50%;box-shadow:0 0 6px rgba(234,179,8,0.6);}
...
.mac-topbar{background:rgba(2,12,20,0.8);backdrop-filter:blur(30px);border-bottom:1px solid rgba(6,182,212,0.1);padding:0.6rem 1.5rem;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:30;}
...
.refresh-btn{background:rgba(6,182,212,0.12);border:1px solid rgba(6,182,212,0.25);border-radius:8px;padding:6px 14px;color:white;font-size:0.75rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.2s;}
.refresh-btn:hover{background:rgba(6,182,212,0.24);transform:translateY(-1px);}
...
.glass-card{background:rgba(4,18,30,0.6);backdrop-filter:blur(20px);border:1px solid rgba(6,182,212,0.1);border-radius:14px;overflow:hidden;}
.section-header{background:linear-gradient(135deg,#041218 0%,#061e2c 50%,#082535 100%);padding:1.25rem 1.5rem;position:relative;overflow:hidden;}
.section-header::before{content:'';position:absolute;top:-80px;right:-40px;width:220px;height:220px;background:rgba(6,182,212,0.06);border-radius:50%;}
.section-header-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(6,182,212,0.12);border:1px solid rgba(6,182,212,0.25);border-radius:50px;padding:3px 10px;font-size:0.65rem;font-weight:700;color:rgba(255,255,255,0.7);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.5rem;}
...
.stat-card.cyan::before  {background:rgba(6,182,212,0.2);}
.stat-card.teal::before  {background:rgba(20,184,166,0.15);}
.stat-card.blue::before  {background:rgba(59,130,246,0.15);}
.stat-card.purple::before{background:rgba(139,92,246,0.15);}
.stat-card.green::before {background:rgba(34,197,94,0.15);}
...
.search-input:focus{border-color:var(--cyan);box-shadow:0 0 0 2px rgba(6,182,212,0.1);}
...
.join-btn{padding:0.35rem 0.85rem;border-radius:7px;font-size:0.72rem;font-weight:700;cursor:pointer;transition:all 0.2s;background:rgba(6,182,212,0.12);border:1px solid rgba(6,182,212,0.3);color:#22d3ee;font-family:inherit;display:inline-flex;align-items:center;gap:4px;}
.join-btn:hover{background:var(--cyan);color:white;border-color:var(--cyan);transform:translateY(-1px);}
...
.form-input:focus{border-color:var(--cyan);box-shadow:0 0 0 3px rgba(6,182,212,0.1);}
.form-input:disabled{opacity:0.4;cursor:not-allowed;}
.form-input option{background:#041218;}
.btn-save{background:linear-gradient(135deg,var(--cyan),var(--teal));color:white;border:none;border-radius:9px;padding:0.7rem 1.8rem;font-size:0.875rem;font-weight:700;cursor:pointer;transition:all 0.2s;font-family:inherit;box-shadow:0 4px 14px rgba(6,182,212,0.3);display:inline-flex;align-items:center;gap:6px;}
...
.alert-notice {background:rgba(234,179,8,0.1);border:1px solid rgba(234,179,8,0.3);color:#fbbf24;}
...
.tab-btn.active{background:rgba(6,182,212,0.18);border-color:rgba(6,182,212,0.35);color:white;}
...
.loc-card:hover{background:rgba(255,255,255,0.07);border-color:rgba(6,182,212,0.25);}
...
.modal-box{background:linear-gradient(135deg,#041218,#020c12);border:1px solid rgba(6,182,212,0.2);border-radius:18px;padding:2rem;width:100%;max-width:420px;margin:1rem;box-shadow:0 24px 64px rgba(0,0,0,0.7);transform:scale(0.95) translateY(10px);transition:all 0.25s cubic-bezier(0.175,0.885,0.32,1.275);text-align:center;}
...
.modal-name{color:#22d3ee;font-weight:700;}
...
.modal-btn-confirm{flex:1;padding:0.7rem 1rem;border-radius:10px;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;border:none;color:white;transition:all 0.2s;background:linear-gradient(135deg,#06B6D4,#14B8A6);box-shadow:0 4px 14px rgba(6,182,212,0.3);}
.modal-btn-confirm:hover{transform:translateY(-1px);}
.modal-btn-confirm.loading{opacity:0.7;pointer-events:none;}
.toast{position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;background:linear-gradient(135deg,#041218,#020c12);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:0.85rem 1.25rem;display:flex;align-items:center;gap:10px;font-size:0.85rem;font-weight:600;color:white;box-shadow:0 8px 32px rgba(0,0,0,0.5);transform:translateY(20px);opacity:0;transition:all 0.3s cubic-bezier(0.175,0.885,0.32,1.275);min-width:260px;}
.toast.show{transform:translateY(0);opacity:1;}
.toast.success{border-color:rgba(6,182,212,0.4);}
...
.mobile-topbar{position:fixed;top:0;left:0;right:0;z-index:50;background:rgba(2,12,20,0.95);backdrop-filter:blur(20px);border-bottom:1px solid rgba(6,182,212,0.1);padding:0.75rem 1rem;display:flex;align-items:center;justify-content:space-between;}
.setup-banner{background:linear-gradient(135deg,rgba(234,179,8,0.12),rgba(234,179,8,0.06));border:1px solid rgba(234,179,8,0.3);border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:12px;}
/* Profile wizard */
...
.wstep-circle.active{background:linear-gradient(135deg,#06B6D4,#14B8A6);border-color:#06B6D4;color:white;box-shadow:0 0 20px rgba(6,182,212,0.5);}
.wstep-circle.done  {background:rgba(6,182,212,0.2);border-color:rgba(6,182,212,0.5);color:#22d3ee;}
...
.wstep-label.active{color:#22d3ee;}
.wstep-label.done  {color:rgba(6,182,212,0.6);}
...
.wstep-line.done{background:rgba(6,182,212,0.4);}
...
.field-input:focus{border-color:var(--cyan);box-shadow:0 0 0 3px rgba(6,182,212,0.12);}
...
.wbtn-next{padding:0.65rem 1.6rem;border-radius:10px;background:linear-gradient(135deg,#06B6D4,#14B8A6);border:none;color:white;font-size:0.85rem;font-weight:800;cursor:pointer;font-family:inherit;transition:all 0.2s;display:flex;align-items:center;gap:6px;box-shadow:0 4px 16px rgba(6,182,212,0.35);}
.wbtn-next:hover{transform:translateY(-1px);box-shadow:0 6px 22px rgba(6,182,212,0.5);}
...
.avatar-swatch.chosen{border-color:white;box-shadow:0 0 0 3px rgba(6,182,212,0.5);transform:scale(1.1);}
...
<aside class="mac-sidebar hidden md:flex flex-col">
    <div class="sidebar-brand">
        ...
        <div><div class="sidebar-brand-text">CEMS</div><div class="sidebar-brand-sub">Beneficiary</div></div>
    </div>
    <div class="sidebar-user">
        <div class="sidebar-avatar"><?php echo strtoupper(substr($_SESSION['fullname']??'B',0,1)); ?></div>
        <div class="sidebar-uname"><?php echo htmlspecialchars(explode(' ',$_SESSION['fullname']??'User')[0]); ?></div>
        <div class="sidebar-urole"><span class="online-dot"></span>Beneficiary</div>
    </div>
    <span class="nav-label">Profile</span>
    <a href="beneficiary.php?section=profile" class="nav-item <?php echo $section==='profile'?'active':''; ?>">
    ...
    <a href="<?php echo $profileIncomplete?'#':'beneficiary.php?section=location'; ?>" class="nav-item <?php echo $section==='location'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-map-pin-line"></i><span>Locations</span>
    </a>
    <a href="<?php echo $profileIncomplete?'#':'beneficiary.php?section=project'; ?>" class="nav-item <?php echo $section==='project'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-folder-2-line"></i><span>Projects</span>
    </a>
    <div class="nav-divider"></div>
    <span class="nav-label">Reports</span>
    <a href="<?php echo $profileIncomplete?'#':'beneficiary.php?section=feedback'; ?>" class="nav-item <?php echo $section==='feedback'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-feedback-line"></i><span>Feedback</span>
    </a>
    <div style="flex:1"></div>
    <a href="beneficiary.php?section=logout" class="logout-btn"><i class="ri-logout-box-line"></i><span>Logout</span></a>
...
<div class="md:hidden mobile-topbar">
    <div class="flex items-center gap-2">
        ...
        <span class="font-bold text-white text-sm">CEMS Beneficiary</span>
    </div>
    <div class="flex items-center gap-2">
...
<div id="mobileSidebar" class="md:hidden hidden fixed inset-0 z-50">
    ...
    <div class="mac-sidebar absolute" style="width:148px;">
        ...
        <div class="sidebar-user"><div class="sidebar-avatar"><?php echo strtoupper(substr($_SESSION['fullname']??'B',0,1)); ?></div><div class="sidebar-uname"><?php echo htmlspecialchars(explode(' ',$_SESSION['fullname']??'User')[0]); ?></div></div>
        <a href="beneficiary.php?section=profile"  class="nav-item <?php echo $section==='profile'?'active':''; ?>"><i class="ri-user-3-line"></i><span>Profile</span><?php if($profileIncomplete): ?><span class="nav-badge"></span><?php endif; ?></a>
        <div class="nav-divider"></div>
        <a href="beneficiary.php?section=location" class="nav-item <?php echo $section==='location'?'active':''; ?>"><i class="ri-map-pin-line"></i><span>Locations</span></a>
        <a href="beneficiary.php?section=project"  class="nav-item <?php echo $section==='project'?'active':''; ?>"><i class="ri-folder-2-line"></i><span>Projects</span></a>
        <div class="nav-divider"></div>
        <a href="beneficiary.php?section=feedback" class="nav-item <?php echo $section==='feedback'?'active':''; ?>"><i class="ri-feedback-line"></i><span>Feedback</span></a>
...
<main class="md:ml-[148px] min-h-screen flex flex-col">
    ...
    <div class="flex-1 p-4 md:p-6 mt-14 md:mt-0 fade-in">
        <?php
        $pageInfo = [
            'profile'  => ['My Profile',    'PROFILE',   'ri-user-3-line',   $profileIncomplete?'Complete your profile to unlock all features':'View and update your beneficiary profile'],
            ...
            'feedback' => ['My Feedback',    'FEEDBACK',  'ri-feedback-line', 'All your submitted project feedback'],
        ];
        ...
        <div class="setup-banner">
            <i class="ri-user-settings-line" style="font-size:1.5rem;color:#fbbf24;flex-shrink:0;"></i>
            <div><div style="font-size:0.9rem;font-weight:700;color:#fbbf24;">⚡ Complete Your Profile Setup</div>
            <div style="font-size:0.78rem;color:rgba(255,255,255,0.5);margin-top:2px;">Fill in your details to unlock Locations, Projects, and Feedback.</div></div>
        </div>
        <?php endif; ?>
        ...
        <div class="glass-card">
            <div class="section-header">
                <div class="section-header-badge">
                    ...
                    <?php if(in_array($section,['project','location','feedback'])): ?><span style="margin-left:4px;">•</span><span id="headerCount">0 Total</span><?php endif; ?>
                ...
                <div class="section-desc"><i class="ri-sparkling-line" style="color:var(--cyan)"></i><?php echo $pi[3]; ?></div>
            </div>
            ...
            $initials = strtoupper(implode('',array_map(fn($w)=>$w[0],array_slice(explode(' ',$urow['fullname']??'B'),0,2))));
            $swatches=[['#06B6D4','#14B8A6'],['#3B82F6','#6366F1'],['#8B5CF6','#A855F7'],['#22C55E','#16a34a'],['#EAB308','#F59E0B'],['#E91E8C','#c4176f'],['#F97316','#EA580C'],['#EC4899','#DB2777']];
            ?>

            <?php if($profileIncomplete): ?>
            <!-- CREATE PROFILE WIZARD -->
            <form id="createProfileForm" action="beneficiary_process.php" method="POST">
            ...
            <div class="wizard-wrap">
                <div class="wizard-steps">
                    ...
                    <div class="wstep"><div class="wstep-circle" id="ws2"><i class="ri-map-pin-line"></i></div><div class="wstep-label" id="wl2">Location</div></div>
                ...
                <div class="wizard-panel active" id="wpanel1">
                    ...
                    <div class="wizard-panel-sub">Tell us about yourself so we can connect you with the right projects.</div>
                    <div style="display:flex;align-items:flex-start;gap:1.5rem;margin-bottom:1.75rem;flex-wrap:wrap;">
                        <div style="display:flex;flex-direction:column;align-items:center;gap:0.75rem;">
                            <div class="avatar-preview" id="avatarPreview" style="background:linear-gradient(135deg,#06B6D4,#14B8A6)">
                                <span id="avatarInitials"><?php echo $initials; ?></span>
                            </div>
                            <span style="font-size:0.62rem;color:rgba(255,255,255,0.3);">Pick a color</span>
                            <div class="avatar-picker">
                                <?php foreach($swatches as $i=>$sw): ?>
                                <div class="avatar-swatch <?php echo $i===0?'chosen':''; ?>" style="background:linear-gradient(135deg,<?php echo $sw[0];?>,<?php echo $sw[1];?>)" data-c1="<?php echo $sw[0];?>" data-c2="<?php echo $sw[1];?>" onclick="pickAvatarColor(this)"></div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div style="flex:1;min-width:220px;">
                            <div class="field-group">
                                ...
                                <input type="text" id="cp_fullname" class="field-input" placeholder="e.g., Juan Dela Cruz" value="<?php echo htmlspecialchars($urow['fullname']??$_SESSION['fullname']??''); ?>" oninput="updateInitials(this.value)">
                            ...
                            <div class="field-group">
                                ...
                                <input type="email" id="cp_email" class="field-input" placeholder="your@email.com" value="<?php echo htmlspecialchars($urow['email']??$_SESSION['email']??''); ?>">
                            </div>
                            <div class="field-group">
                ...
                <div class="wizard-panel" id="wpanel2">
                    <div class="wizard-panel-title">📍 Your Community</div>
                    <div class="wizard-panel-sub">Tell us which community or barangay you belong to.</div>
                    <div class="field-group">
                        <label class="field-label"><i class="ri-community-line" style="color:var(--cyan);margin-right:4px;"></i>Community / Barangay / Address <span style="color:#f87171;">*</span></label>
                        <input type="text" id="cp_assignments" class="field-input" placeholder="e.g., Brgy. San Jose, Cebu City">
                        <div class="field-error" id="err_assignments">Please enter your community or address.</div>
                    </div>
                    <div style="margin-top:0.5rem;">
                        <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:rgba(255,255,255,0.25);margin-bottom:0.6rem;">Common entries:</div>
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
                            <?php foreach(['Brgy. Poblacion','Brgy. San Jose','Brgy. Labangon','Brgy. Mabolo','Brgy. Luz','Brgy. Basak','Brgy. Guadalupe','Indigenous Community'] as $t): ?>
                            <button type="button" onclick="document.getElementById('cp_assignments').value='<?php echo $t; ?>'" style="padding:5px 12px;border-radius:20px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);font-size:0.72rem;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.15s;" onmouseover="this.style.background='rgba(6,182,212,0.15)';this.style.borderColor='rgba(6,182,212,0.4)';this.style.color='#22d3ee'" onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.1)';this.style.color='rgba(255,255,255,0.5)'"><?php echo $t; ?></button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                ...
                <div class="wizard-panel" id="wpanel4">
                    ...
                    <div class="wizard-panel-sub">Confirm your details to unlock Locations, Projects, and Feedback.</div>
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:1.25rem;margin-bottom:1.5rem;">
                        <div style="display:flex;align-items:center;gap:14px;margin-bottom:1.25rem;padding-bottom:1rem;border-bottom:1px solid rgba(255,255,255,0.06);">
                            <div id="reviewAvatar" style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#06B6D4,#14B8A6);display:flex;align-items:center;justify-content:center;font-size:1.4rem;font-weight:800;color:white;border:3px solid rgba(255,255,255,0.1);flex-shrink:0;"></div>
                            <div>
                                <div id="reviewName" style="font-size:1.1rem;font-weight:800;color:white;"></div>
                                <div style="font-size:0.75rem;color:rgba(255,255,255,0.4);margin-top:2px;display:flex;align-items:center;gap:5px;"><i class="ri-heart-line" style="color:var(--cyan)"></i>Beneficiary</div>
                            </div>
                            <div style="margin-left:auto;padding:4px 12px;background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);border-radius:50px;font-size:0.65rem;font-weight:800;color:#22d3ee;">NEW PROFILE</div>
                        </div>
                        ...
                        <div class="review-row"><div class="review-icon" style="background:rgba(6,182,212,0.15);"><i class="ri-community-line" style="color:#22d3ee;"></i></div><div class="review-key">Community</div><div class="review-val" id="rv_dept">—</div></div>
                    ...
                    <div style="background:linear-gradient(135deg,rgba(6,182,212,0.1),rgba(20,184,166,0.06));border:1px solid rgba(6,182,212,0.2);border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.5rem;">
                        <div style="font-size:0.72rem;font-weight:800;text-transform:uppercase;letter-spacing:0.1em;color:#22d3ee;margin-bottom:0.75rem;display:flex;align-items:center;gap:6px;"><i class="ri-unlock-line"></i>Features you'll unlock</div>
                        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
                            <?php foreach(['📍 View Locations','📁 View Projects','🤝 Join Projects','💬 Submit Feedback','✏️ Update Profile','🔒 Change Password'] as $f): ?>
                            <div style="font-size:0.78rem;color:rgba(255,255,255,0.6);display:flex;align-items:center;gap:6px;"><span style="color:#22d3ee;">✓</span><?php echo $f; ?></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                        <button type="button" class="wbtn-back" onclick="wizardBack(4)"><i class="ri-arrow-left-line"></i> Back</button>
                        <button type="button" class="wbtn-next" id="createBtn" onclick="submitCreateProfile()" style="padding-left:2rem;padding-right:2rem;">
            ...
            <script>
            let currentWizStep=1,chosenC1='#06B6D4',chosenC2='#14B8A6';
            ...
            function goToStep(n){
                ...
                const icons=['ri-user-line','ri-map-pin-line','ri-lock-line','ri-check-line'];
            ...
            function populateReview(){
                ...
                document.getElementById('rv_pw').textContent=hasPw?'•••••••• (new)':'Keeping current';
            ...
            <?php else: ?>
            <!-- UPDATE PROFILE -->
            <div style="background:linear-gradient(135deg,#041218 0%,#061e2c 60%,#082535 100%);padding:2rem 2rem 0;position:relative;overflow:hidden;">
                <div style="position:absolute;top:-60px;right:-40px;width:220px;height:220px;background:rgba(6,182,212,0.06);border-radius:50%;"></div>
                <div style="display:flex;align-items:flex-end;gap:1.5rem;position:relative;z-index:1;">
                    <div style="position:relative;flex-shrink:0;">
                        <div id="avatarCircle" onclick="document.getElementById('avatarOptions').classList.toggle('hidden')" style="width:90px;height:90px;border-radius:50%;background:linear-gradient(135deg,#06B6D4,#14B8A6);display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:800;color:white;border:4px solid rgba(255,255,255,0.1);box-shadow:0 8px 32px rgba(6,182,212,0.3);cursor:pointer;transition:all 0.2s;" title="Click to change color">
                            <?php echo $initials; ?>
                        </div>
                        <div style="position:absolute;bottom:2px;right:2px;width:18px;height:18px;background:var(--cyan);border-radius:50%;border:2px solid #041218;box-shadow:0 0 8px rgba(6,182,212,0.6);"></div>
                        <div id="avatarOptions" class="hidden" style="position:absolute;top:100%;left:0;margin-top:8px;background:rgba(4,18,24,0.95);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:0.75rem;display:flex;gap:6px;z-index:10;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
                    ...
                    <div style="padding-bottom:1.25rem;flex:1;">
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                            <h2 style="font-size:1.5rem;font-weight:800;color:white;"><?php echo htmlspecialchars($urow['fullname']??'User'); ?></h2>
                            <span style="background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);border-radius:50px;padding:2px 12px;font-size:0.68rem;font-weight:700;color:#22d3ee;">● Active</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:1rem;margin-top:6px;flex-wrap:wrap;">
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="ri-heart-line" style="color:var(--cyan)"></i>Beneficiary</span>
                            <?php if(!empty($urow['assignments'])): ?><span style="font-size:0.8rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="ri-community-line" style="color:#60a5fa"></i><?php echo htmlspecialchars($urow['assignments']); ?></span><?php endif; ?>
                    ...
                    <div style="padding-bottom:1.25rem;">
                        <button onclick="document.getElementById('profileForm').scrollIntoView({behavior:'smooth'})" style="background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);border-radius:9px;padding:7px 14px;color:#22d3ee;font-size:0.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.2s;" onmouseover="this.style.background='rgba(6,182,212,0.25)'" onmouseout="this.style.background='rgba(6,182,212,0.15)'"><i class="ri-edit-line"></i>Edit Profile</button>
                    </div>
                </div>
            </div>
            ...
            <!-- Info Cards -->
            <div class="stat-grid">
                <div class="stat-card cyan"><div><div class="stat-label">Role</div><div class="stat-value" style="font-size:0.85rem;margin-top:4px;line-height:1.3;">Beneficiary</div><div class="stat-sub">Community member</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#06B6D4,#14B8A6)"><i class="ri-user-heart-line"></i></div></div>
                <div class="stat-card blue"><div><div class="stat-label">Community</div><div class="stat-value" style="font-size:0.72rem;margin-top:4px;line-height:1.4;"><?php echo htmlspecialchars(substr($urow['assignments']??'Not set',0,22)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#3B82F6,#6366F1)"><i class="ri-community-line"></i></div></div>
                <div class="stat-card teal"><div><div class="stat-label">Email</div><div class="stat-value" style="font-size:0.68rem;margin-top:4px;line-height:1.4;word-break:break-all;"><?php echo htmlspecialchars(substr($urow['email']??'Not set',0,24)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#14B8A6,#0D9488)"><i class="ri-mail-line"></i></div></div>
                <div class="stat-card purple"><div><div class="stat-label">Phone</div><div class="stat-value" style="font-size:0.8rem;margin-top:4px;"><?php echo htmlspecialchars($urow['phone']??'Not set'); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#8B5CF6,#7C3AED)"><i class="ri-phone-line"></i></div></div>
            </div>
            ...
            <div class="content-area" id="profileForm">
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:1.25rem;padding-bottom:0.75rem;border-bottom:1px solid rgba(255,255,255,0.07);">
                    <div style="width:32px;height:32px;border-radius:8px;background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.25);display:flex;align-items:center;justify-content:center;"><i class="ri-edit-2-line" style="color:var(--cyan);font-size:0.95rem;"></i></div>
                    <div><div style="font-size:0.9rem;font-weight:700;color:white;">Edit Information</div><div style="font-size:0.7rem;color:rgba(255,255,255,0.35);">Update your personal details below</div></div>
                </div>

                <form id="profileFormEl" action="beneficiary_process.php" method="POST" style="max-width:680px;">
                    ...
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:1.25rem;margin-bottom:1rem;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-group"><label class="form-label"><i class="ri-user-line" style="color:var(--cyan);margin-right:4px;"></i>Full Name</label><input type="text" name="fullname" id="inp_fullname" class="form-input" value="<?php echo htmlspecialchars($urow['fullname']??''); ?>" required></div>
                            ...
                            <div class="form-group"><label class="form-label"><i class="ri-heart-line" style="color:var(--cyan);margin-right:4px;"></i>Role</label><input type="text" class="form-input" value="Beneficiary" disabled style="opacity:0.4;"></div>
                            <div class="form-group" style="grid-column:1/-1;"><label class="form-label"><i class="ri-community-line" style="color:#60a5fa;margin-right:4px;"></i>Community / Barangay</label><input type="text" name="assignments" id="inp_assignments" class="form-input" value="<?php echo htmlspecialchars($urow['assignments']??''); ?>"></div>
                        </div>
                    </div>
            ...
            <div id="saveConfirmModal" style="display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.8);backdrop-filter:blur(6px);align-items:center;justify-content:center;">
                <div style="background:linear-gradient(135deg,#041218,#020c12);border:1px solid rgba(6,182,212,0.2);border-radius:18px;padding:2rem;width:100%;max-width:420px;margin:1rem;box-shadow:0 24px 64px rgba(0,0,0,0.7);text-align:center;">
                    <div style="width:64px;height:64px;border-radius:50%;background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;font-size:1.8rem;color:var(--cyan);"><i class="ri-save-3-line"></i></div>
                    ...
                    <div style="display:flex;gap:0.75rem;">
                        ...
                        <button onclick="submitProfileForm()" style="flex:1;padding:0.7rem;border-radius:10px;background:linear-gradient(135deg,#06B6D4,#14B8A6);border:none;color:white;font-weight:700;cursor:pointer;font-family:inherit;font-size:0.85rem;box-shadow:0 4px 14px rgba(6,182,212,0.3);"><i class="ri-check-line"></i> Yes, Save</button>
                    </div>
                </div>
            </div>
            <script>
            ...
            function openSaveConfirm(){const name=document.getElementById('inp_fullname').value,email=document.getElementById('inp_email').value,phone=document.getElementById('inp_phone').value,dept=document.getElementById('inp_assignments').value,hasPw=document.getElementById('inp_password').value!=='';let p=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Name:</span><span style="color:white;font-weight:600;">${name}</span></div>`;p+=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Email:</span><span style="color:white;font-weight:600;">${email||'—'}</span></div>`;p+=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Phone:</span><span style="color:white;font-weight:600;">${phone||'—'}</span></div>`;p+=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Community:</span><span style="color:white;font-weight:600;">${dept||'—'}</span></div>`;if(hasPw)p+=`<div style="margin-top:6px;color:#fbbf24;font-size:0.72rem;"><i class="ri-lock-line"></i> Password will also be updated</div>`;document.getElementById('changePreview').innerHTML=p;document.getElementById('saveConfirmModal').style.display='flex';document.body.style.overflow='hidden';}
            ...
            <div class="stat-grid">
                <div class="stat-card cyan"><div><div class="stat-label">Total</div><div class="stat-value" id="pStatTotal">0</div><div class="stat-sub">All projects</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#06B6D4,#14B8A6)"><i class="ri-folder-2-line"></i></div></div>
                <div class="stat-card blue"><div><div class="stat-label">Active</div><div class="stat-value" id="pStatActive">0</div><div class="stat-sub" style="color:#60a5fa">• In progress</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#3B82F6,#6366F1)"><i class="ri-play-circle-line"></i></div></div>
                <div class="stat-card teal"><div><div class="stat-label">Planning</div><div class="stat-value" id="pStatPlanning">0</div><div class="stat-sub">Scheduled</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#14B8A6,#0D9488)"><i class="ri-calendar-check-line"></i></div></div>
                <div class="stat-card green"><div><div class="stat-label">Completed</div><div class="stat-value" id="pStatCompleted">0</div><div class="stat-sub" style="color:#4ade80">✓ Delivered</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#22C55E,#16a34a)"><i class="ri-checkbox-circle-line"></i></div></div>
            </div>
            <div class="content-area">
                <div class="tab-wrap">
                ...
                <div id="project-view">
                    <div class="filter-bar"><div class="search-wrap"><i class="ri-search-line"></i><input type="text" id="projSearch" class="search-input" placeholder="Search projects by name or status..."></div></div>
                ...
                <div id="project-feedback" class="hidden" style="max-width:520px;">
                    ...
                    <form method="POST" action="beneficiary_process.php" class="space-y-4">
                        ...
                        <div class="form-group"><label class="form-label">Your Feedback</label><textarea name="feedback" rows="4" class="form-input" placeholder="Share your experience as a community member..." style="resize:none;"></textarea></div>
            ...
            <div class="content-area">
                <?php if(empty($evals)): ?>
                <div class="text-center py-16"><i class="ri-feedback-line text-5xl" style="color:rgba(255,255,255,0.08)"></i><p class="mt-4 text-sm" style="color:rgba(255,255,255,0.35)">No feedback submitted yet.</p><p class="text-xs mt-1" style="color:rgba(255,255,255,0.2)">Go to Projects to submit feedback.</p></div>
                <?php else: ?>
        ...
        <div class="status-bar"><span>© <?php echo date('Y'); ?> CEMS. All rights reserved.</span><span>Beneficiary Portal &nbsp;•&nbsp; v2.0</span></div>
    </div>
</main>
...
<div class="modal-overlay" id="joinModal">
    <div class="modal-box">
        <div class="modal-icon-wrap" style="background:rgba(6,182,212,0.15);border:1px solid rgba(6,182,212,0.3);"><i class="ri-folder-add-line" style="color:#22d3ee;"></i></div>
        <div class="modal-title">Join Project</div>
        <div class="modal-msg">Are you sure you want to join<br><span class="modal-name" id="joinProjName"></span>?</div>
        <div class="modal-btns">
            <button class="modal-btn-cancel" onclick="closeJoinModal()"><i class="ri-close-line"></i> Cancel</button>
            <button class="modal-btn-confirm" id="joinConfirmBtn" onclick="confirmJoin()"><i class="ri-check-line"></i> Join Project</button>
        </div>
    </div>
</div>
...
document.querySelectorAll('a.nav-item:not(.locked)').forEach(link=>{
    link.addEventListener('click',e=>{
        if(link.href.includes('beneficiary.php')&&!link.href.includes('logout')){
            e.preventDefault();const main=document.querySelector('main');
            main.style.transition='opacity 0.2s,transform 0.2s';main.style.opacity='0';main.style.transform='translateY(8px)';
...
function showToast(msg,type='success'){
    ...
    icon.style.color=type==='success'?'#22d3ee':'#f87171';
...
// Join Project Modal
let _joinProjId=0;
function openJoinProject(id,name){
    _joinProjId=id;
    document.getElementById('joinProjName').textContent='"'+name+'"';
...
function confirmJoin(){
    if(!_joinProjId)return;
    const btn=document.getElementById('joinConfirmBtn');btn.classList.add('loading');btn.textContent='Joining...';
    const fd=new FormData();fd.append('action','join_project');fd.append('project_id',_joinProjId);
    fetch('beneficiary_process.php',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
        closeJoinModal();
        if(d.status==='success')showToast(d.message||'Joined successfully! 🎉','success');
        else showToast(d.message||'Something went wrong.','error');
    }).catch(()=>{closeJoinModal();showToast('Network error. Please try again.','error');}).finally(()=>{btn.classList.remove('loading');btn.innerHTML='<i class="ri-check-line"></i> Join Project';});
}
...
function renderProjects(data){
    ...
    const sc={'Active':'badge-cyan','Planning':'badge-blue','Completed':'badge-green','On Hold':'badge-yellow'};
    document.getElementById('projectsContent').innerHTML=`<div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Project</th><th>Timeline</th><th>Status</th><th>Action</th></tr></thead><tbody>${data.map(p=>`<tr><td><div class="flex items-center gap-3"><div class="row-icon" style="background:linear-gradient(135deg,#06B6D4,#8B5CF6)"><i class="ri-folder-2-line"></i></div><div class="row-name">${p.name||'—'}</div></div></td><td class="row-sub">${p.start_date||'—'}</td><td><span class="badge ${sc[p.status]||'badge-gray'}">${p.status||'—'}</span></td><td><button class="join-btn" onclick="openJoinProject(${p.id},'${(p.name||'').replace(/'/g,"\\'")}')"><i class="ri-user-add-line"></i> Join</button></td></tr>`).join('')}</tbody></table><div class="mt-3 text-xs" style="color:rgba(255,255,255,0.3)">Total: <strong style="color:white">${data.length}</strong> projects</div></div>`;
}

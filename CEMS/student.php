...
// Profile incomplete = phone AND course/year both empty
...
<title>CEMS - Student Portal</title>
...
body {
    min-height: 100vh;
    background: linear-gradient(145deg, #000510 0%, #0a1628 25%, #0d1f3c 50%, #0a1628 75%, #000510 100%);
...
.sidebar-brand-icon { width:28px; height:28px; border-radius:7px; background:linear-gradient(135deg,var(--blue),var(--cyan)); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
...
.sidebar-brand-sub  { font-size:0.55rem; color:rgba(21,101,192,0.8); font-weight:600; }
.sidebar-user { background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:0.6rem; margin-bottom:1.2rem; display:flex; flex-direction:column; align-items:center; gap:4px; }
.sidebar-avatar { width:36px; height:36px; border-radius:9px; background:linear-gradient(135deg,var(--blue),var(--cyan)); display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.95rem; color:white; }
...
.online-dot { width:5px; height:5px; border-radius:50%; background:var(--cyan); box-shadow:0 0 5px var(--cyan); animation:glow 2s infinite; display:inline-block; }
@keyframes glow { 0%,100%{box-shadow:0 0 0 0 rgba(0,180,216,0.5);} 50%{box-shadow:0 0 0 4px rgba(0,180,216,0);} }
...
.nav-item.active { background:rgba(21,101,192,0.22); border-color:rgba(21,101,192,0.4); color:white; font-weight:700; box-shadow:0 0 12px rgba(21,101,192,0.2); }
...
.nav-badge { position:absolute; top:4px; right:4px; width:8px; height:8px; background:#F5A623; border-radius:50%; box-shadow:0 0 6px rgba(245,166,35,0.6); }
...
.mac-topbar { background:rgba(5,10,25,0.8); backdrop-filter:blur(30px); border-bottom:1px solid rgba(21,101,192,0.15); padding:0.6rem 1.5rem; display:flex; align-items:center; justify-content:space-between; position:sticky; top:0; z-index:30; }
...
.refresh-btn { background:rgba(21,101,192,0.15); border:1px solid rgba(21,101,192,0.3); border-radius:8px; padding:6px 14px; color:white; font-size:0.75rem; font-weight:600; cursor:pointer; display:flex; align-items:center; gap:5px; transition:all 0.2s; }
.refresh-btn:hover { background:rgba(21,101,192,0.28); transform:translateY(-1px); }
...
.glass-card { background:rgba(10,20,50,0.55); backdrop-filter:blur(20px); border:1px solid rgba(21,101,192,0.15); border-radius:14px; overflow:hidden; }
.section-header { background:linear-gradient(135deg,#0d1b2a 0%,#1a2744 50%,#243050 100%); padding:1.25rem 1.5rem; position:relative; overflow:hidden; }
.section-header::before { content:''; position:absolute; top:-80px; right:-40px; width:220px; height:220px; background:rgba(21,101,192,0.08); border-radius:50%; }
.section-header-badge { display:inline-flex; align-items:center; gap:5px; background:rgba(21,101,192,0.15); border:1px solid rgba(21,101,192,0.25); border-radius:50px; padding:3px 10px; font-size:0.65rem; font-weight:700; color:rgba(255,255,255,0.7); letter-spacing:0.08em; text-transform:uppercase; margin-bottom:0.5rem; }
...
.stat-card.blue::before   { background:rgba(21,101,192,0.2); }
.stat-card.cyan::before   { background:rgba(0,180,216,0.15); }
.stat-card.purple::before { background:rgba(123,47,190,0.15); }
...
.search-input:focus { border-color:var(--cyan); box-shadow:0 0 0 2px rgba(0,180,216,0.1); }
...
.badge-blue   { background:rgba(21,101,192,0.15); color:#60a5fa; border:1px solid rgba(21,101,192,0.3); }
.badge-cyan   { background:rgba(0,180,216,0.15); color:#67e8f9; border:1px solid rgba(0,180,216,0.3); }
...
.join-btn { padding:0.35rem 0.85rem; border-radius:7px; font-size:0.72rem; font-weight:700; cursor:pointer; transition:all 0.2s; background:rgba(21,101,192,0.15); border:1px solid rgba(21,101,192,0.35); color:#60a5fa; font-family:inherit; }
.join-btn:hover { background:var(--blue); color:white; border-color:var(--blue); transform:translateY(-1px); }
...
.form-input option { background:#0d1b2a; }
.btn-save { background:linear-gradient(135deg,var(--blue),var(--cyan)); color:white; border:none; border-radius:9px; padding:0.7rem 1.8rem; font-size:0.875rem; font-weight:700; cursor:pointer; transition:all 0.2s; font-family:inherit; box-shadow:0 4px 14px rgba(21,101,192,0.35); display:inline-flex; align-items:center; gap:6px; }
.btn-save:hover { transform:translateY(-1px); box-shadow:0 6px 20px rgba(21,101,192,0.5); }
...
.tab-btn.active { background:rgba(21,101,192,0.2); border-color:rgba(21,101,192,0.4); color:white; }
...
.loc-card:hover { background:rgba(255,255,255,0.07); border-color:rgba(21,101,192,0.3); }
.modal-overlay { position:fixed; inset:0; z-index:999; background:rgba(0,0,0,0.75); backdrop-filter:blur(6px); display:flex; align-items:center; justify-content:center; opacity:0; pointer-events:none; transition:opacity 0.2s; }
.modal-overlay.show { opacity:1; pointer-events:all; }
.modal-box { background:linear-gradient(135deg,#0d1b2a,#0a1424); border:1px solid rgba(21,101,192,0.2); border-radius:18px; padding:2rem; width:100%; max-width:400px; margin:1rem; box-shadow:0 24px 64px rgba(0,0,0,0.6); transform:scale(0.95) translateY(10px); transition:all 0.25s cubic-bezier(0.175,0.885,0.32,1.275); text-align:center; }
...
::-webkit-scrollbar-thumb { background:rgba(21,101,192,0.3); border-radius:20px; }
...
.mobile-topbar { position:fixed; top:0; left:0; right:0; z-index:50; background:rgba(5,10,25,0.95); backdrop-filter:blur(20px); border-bottom:1px solid rgba(21,101,192,0.15); padding:0.75rem 1rem; display:flex; align-items:center; justify-content:space-between; }
/* Wizard */
...
.wstep-circle.active { background:linear-gradient(135deg,#1565C0,#00B4D8); border-color:#00B4D8; color:white; box-shadow:0 0 20px rgba(0,180,216,0.5); }
.wstep-circle.done   { background:rgba(0,180,216,0.2); border-color:rgba(0,180,216,0.5); color:#67e8f9; }
...
.wstep-label.active { color:#67e8f9; }
.wstep-label.done   { color:rgba(0,180,216,0.6); }
...
.wstep-line.done { background:rgba(0,180,216,0.4); }
...
.field-input:focus { border-color:#00B4D8; box-shadow:0 0 0 3px rgba(0,180,216,0.12); background:rgba(0,0,0,0.45); }
...
.wbtn-next { padding:0.65rem 1.6rem; border-radius:10px; background:linear-gradient(135deg,#1565C0,#00B4D8); border:none; color:white; font-size:0.85rem; font-weight:800; cursor:pointer; font-family:inherit; transition:all 0.2s; display:flex; align-items:center; gap:6px; box-shadow:0 4px 16px rgba(21,101,192,0.4); }
.wbtn-next:hover { transform:translateY(-1px); box-shadow:0 6px 22px rgba(21,101,192,0.55); }
...
.avatar-swatch.chosen { border-color:white; box-shadow:0 0 0 3px rgba(0,180,216,0.5); transform:scale(1.1); }
...
<aside class="mac-sidebar hidden md:flex flex-col">
    <div class="sidebar-brand">
        ...
        <div><div class="sidebar-brand-text">CEMS</div><div class="sidebar-brand-sub">Student Portal</div></div>
    </div>
    <div class="sidebar-user">
        <div class="sidebar-avatar"><?php echo strtoupper(substr($_SESSION['fullname']??'S',0,1)); ?></div>
        <div class="sidebar-uname"><?php echo htmlspecialchars(explode(' ',$_SESSION['fullname']??'Student')[0]); ?></div>
        <div class="sidebar-urole"><span class="online-dot"></span>Student</div>
    </div>
    <span class="nav-label">Profile</span>
    <a href="student.php?section=profile" class="nav-item <?php echo $section==='profile'?'active':''; ?>">
    ...
    <a href="<?php echo $profileIncomplete?'#':'student.php?section=location'; ?>" class="nav-item <?php echo $section==='location'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-map-pin-line"></i><span>Locations</span>
    </a>
    <a href="<?php echo $profileIncomplete?'#':'student.php?section=activity'; ?>" class="nav-item <?php echo $section==='activity'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-calendar-event-line"></i><span>Activities</span>
    </a>
    <a href="<?php echo $profileIncomplete?'#':'student.php?section=project'; ?>" class="nav-item <?php echo $section==='project'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-folder-2-line"></i><span>Projects</span>
    </a>
    <div class="nav-divider"></div>
    <span class="nav-label">Reports</span>
    <a href="<?php echo $profileIncomplete?'#':'student.php?section=feedback'; ?>" class="nav-item <?php echo $section==='feedback'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-feedback-line"></i><span>Feedback</span>
    </a>
    <div style="flex:1"></div>
    <a href="student.php?section=logout" class="logout-btn"><i class="ri-logout-box-line"></i><span>Logout</span></a>
...
<div class="md:hidden mobile-topbar">
    <div class="flex items-center gap-2">
        ...
        <span class="font-bold text-white text-sm">CEMS Student</span>
    </div>
    <div class="flex items-center gap-2">
...
<div id="mobileSidebar" class="md:hidden hidden fixed inset-0 z-50">
    ...
    <div class="mac-sidebar absolute" style="width:148px;">
        ...
        <a href="student.php?section=profile"  class="nav-item <?php echo $section==='profile'?'active':''; ?>"><i class="ri-user-3-line"></i><span>Profile</span><?php if($profileIncomplete): ?><span class="nav-badge"></span><?php endif; ?></a>
        <div class="nav-divider"></div>
        <a href="student.php?section=location" class="nav-item <?php echo $section==='location'?'active':''; ?>"><i class="ri-map-pin-line"></i><span>Locations</span></a>
        <a href="student.php?section=activity" class="nav-item <?php echo $section==='activity'?'active':''; ?>"><i class="ri-calendar-event-line"></i><span>Activities</span></a>
        <a href="student.php?section=project"  class="nav-item <?php echo $section==='project'?'active':''; ?>"><i class="ri-folder-2-line"></i><span>Projects</span></a>
        <div class="nav-divider"></div>
        <a href="student.php?section=feedback" class="nav-item <?php echo $section==='feedback'?'active':''; ?>"><i class="ri-feedback-line"></i><span>Feedback</span></a>
...
<main class="md:ml-[148px] min-h-screen flex flex-col">
    <div class="mac-topbar hidden md:flex">
        ...
        <div class="flex items-center gap-3">
            <span class="sys-status"><span class="w-2 h-2 rounded-full animate-pulse" style="background:var(--cyan);box-shadow:0 0 6px var(--cyan)"></span>System Online</span>
    ...
    <div class="flex-1 p-4 md:p-6 mt-14 md:mt-0 fade-in">
        ...
        <?php
        $pageInfo = [
            'profile'  => ['My Profile',    'PROFILE',    'ri-user-3-line',         $profileIncomplete ? 'Complete your profile to unlock all features' : 'View and update your student profile'],
        ...
        <div class="glass-card">
            <div class="section-header">
                <div class="section-header-badge">
                    ...
                    <?php echo $pi[1]; ?>
                    <?php if(in_array($section,['activity','project','location','feedback'])): ?>
                    <span style="margin-left:4px;">•</span><span id="headerCount">0 Total</span>
                    <?php endif; ?>
                    <?php if($profileIncomplete && $section==='profile'): ?>
                    <span style="margin-left:4px;color:#fbbf24;">• Setup Required</span>
                    <?php endif; ?>
                </div>
                <div class="section-title">
                    <?php echo $pi[0]; ?><span>/ <?php echo ucfirst($section); ?></span>
                </div>
                <div class="section-desc">
                    <i class="ri-sparkling-line" style="color:var(--cyan)"></i><?php echo $pi[3]; ?>
                </div>
            </div>

            <?php if($section === 'profile'): ?>
            <?php
            $initials = strtoupper(implode('', array_map(fn($w)=>$w[0], array_slice(explode(' ',$urow['fullname']??'S'), 0, 2))));
            $swatches = [['#1565C0','#00B4D8'],['#1DB954','#00B4A0'],['#8B2FC9','#6d24a8'],['#E91E8C','#c4176f'],['#F5A623','#e08c0a'],['#E32636','#b81c28'],['#FF6B35','#e55a24'],['#00B4A0','#00897B']];
            ?>

            <?php if($profileIncomplete): ?>
            <!-- ══════════════════════════════════════
                 CREATE PROFILE — 4-step wizard
            ══════════════════════════════════════ -->
            <form id="createProfileForm" action="student_process.php" method="POST">
            ...
            <div class="wizard-wrap">
                ...
                <div class="wizard-steps">
                    ...
                    <div class="wstep"><div class="wstep-circle" id="ws2"><i class="ri-book-line"></i></div><div class="wstep-label" id="wl2">Course</div></div>
                ...
                <div class="wizard-panel active" id="wpanel1">
                    <div class="wizard-panel-title">👋 Welcome! Let's set up your profile</div>
                    <div class="wizard-panel-sub">Tell us who you are — this is how the community will know you.</div>
                    <div style="display:flex;align-items:flex-start;gap:1.5rem;margin-bottom:1.75rem;flex-wrap:wrap;">
                        <div style="display:flex;flex-direction:column;align-items:center;gap:0.75rem;">
                            <div class="avatar-preview" id="avatarPreview" style="background:linear-gradient(135deg,#1565C0,#00B4D8)">
                        ...
                        <div style="flex:1;min-width:220px;">
                            <div class="field-group">
                                <label class="field-label"><i class="ri-user-line" style="color:var(--cyan);margin-right:4px;"></i>Full Name <span style="color:#f87171;">*</span></label>
                                <input type="text" id="cp_fullname" class="field-input" placeholder="e.g., Juan Dela Cruz"
                                       value="<?php echo htmlspecialchars($urow['fullname']??$_SESSION['fullname']??''); ?>"
                                       oninput="updateInitials(this.value)">
                                <div class="field-error" id="err_fullname">Please enter your full name.</div>
                            </div>
                            <div class="field-group">
                                <label class="field-label"><i class="ri-mail-line" style="color:#60a5fa;margin-right:4px;"></i>Email Address</label>
                                <input type="email" id="cp_email" class="field-input" placeholder="juan@school.edu.ph"
                                       value="<?php echo htmlspecialchars($urow['email']??$_SESSION['email']??''); ?>">
                                <div class="field-hint"><i class="ri-information-line"></i>Optional but recommended</div>
                            </div>
                            <div class="field-group">
                ...
                <div class="wizard-panel" id="wpanel2">
                    <div class="wizard-panel-title">📚 Your Course & Year Level</div>
                    <div class="wizard-panel-sub">What are you studying? This helps match you to the right programs.</div>
                    <div class="field-group">
                        <label class="field-label"><i class="ri-book-line" style="color:var(--cyan);margin-right:4px;"></i>Course / Year Level <span style="color:#f87171;">*</span></label>
                        <input type="text" id="cp_assignments" class="field-input" placeholder="e.g., BS Computer Science - 3rd Year">
                        <div class="field-error" id="err_assignments">Please enter your course and year level.</div>
                        <div class="field-hint"><i class="ri-information-line"></i>You can update this later</div>
                    </div>
                    <div style="margin-top:0.5rem;">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
                            <?php foreach(['BS Computer Science','BS Information Technology','BS Education','BS Nursing','BS Engineering','BS Business Administration','BS Accountancy','AB Communication','Senior High School'] as $c): ?>
                            <button type="button" onclick="document.getElementById('cp_assignments').value='<?php echo $c; ?>'"
                                    ...
                                    onmouseover="this.style.background='rgba(21,101,192,0.2)';this.style.borderColor='rgba(0,180,216,0.4)';this.style.color='#67e8f9'"
                                    onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.1)';this.style.color='rgba(255,255,255,0.5)'"><?php echo $c; ?></button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                ...
                <div class="wizard-panel" id="wpanel3">
                    ...
                    <div style="margin-top:0.75rem;padding:0.75rem 1rem;background:rgba(245,166,35,0.06);border:1px solid rgba(245,166,35,0.2);border-radius:10px;display:flex;align-items:center;gap:8px;">
                        ...
                        <span style="font-size:0.75rem;color:rgba(255,255,255,0.45);">You can skip password setup and use your existing credentials.</span>
                    </div>
                    <div class="wizard-nav">
                ...
                <div class="wizard-panel" id="wpanel4">
                    ...
                    <div class="wizard-panel-sub">Everything look good? Hit "Create Profile" to get started.</div>
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:1.25rem;margin-bottom:1.5rem;">
                        <div style="display:flex;align-items:center;gap:14px;margin-bottom:1.25rem;padding-bottom:1rem;border-bottom:1px solid rgba(255,255,255,0.06);">
                            <div id="reviewAvatar" style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#1565C0,#00B4D8);display:flex;align-items:center;justify-content:center;font-size:1.4rem;font-weight:800;color:white;border:3px solid rgba(255,255,255,0.1);flex-shrink:0;"></div>
                            <div>
                                <div id="reviewName" style="font-size:1.1rem;font-weight:800;color:white;"></div>
                                <div style="font-size:0.75rem;color:rgba(255,255,255,0.4);margin-top:2px;display:flex;align-items:center;gap:5px;"><i class="ri-user-3-line" style="color:var(--cyan)"></i>Student</div>
                            </div>
                            <div style="margin-left:auto;padding:4px 12px;background:rgba(0,180,216,0.15);border:1px solid rgba(0,180,216,0.3);border-radius:50px;font-size:0.65rem;font-weight:800;color:#67e8f9;">NEW PROFILE</div>
                        </div>
                        ...
                        <div class="review-row"><div class="review-icon" style="background:rgba(0,180,216,0.15);"><i class="ri-book-line" style="color:#67e8f9;"></i></div><div class="review-key">Course</div><div class="review-val" id="rv_dept">—</div></div>
                    ...
                    <div style="background:linear-gradient(135deg,rgba(21,101,192,0.1),rgba(0,180,216,0.06));border:1px solid rgba(0,180,216,0.2);border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.5rem;">
                        <div style="font-size:0.72rem;font-weight:800;text-transform:uppercase;letter-spacing:0.1em;color:#67e8f9;margin-bottom:0.75rem;display:flex;align-items:center;gap:6px;"><i class="ri-unlock-line"></i>Features you'll unlock</div>
                        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
                            <?php foreach(['📍 View Locations','📅 View Activities','✋ Join Activities','📁 View Projects','🤝 Join Projects','💬 Submit Feedback'] as $f): ?>
                            <div style="font-size:0.78rem;color:rgba(255,255,255,0.6);display:flex;align-items:center;gap:6px;"><span style="color:#67e8f9;">✓</span><?php echo $f; ?></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                        ...
                        <button type="button" class="wbtn-next" id="createBtn" onclick="submitCreateProfile()"
                                style="background:linear-gradient(135deg,#1565C0,#00B4D8);padding-left:2rem;padding-right:2rem;">
            ...
            <script>
            let currentWizStep=1, chosenC1='#1565C0', chosenC2='#00B4D8';
            ...
            function goToStep(n){
                ...
                const icons=['ri-user-line','ri-book-line','ri-lock-line','ri-check-line'];
                for(let i=1;i<=4;i++){
                    ...
                    if(i<n){c.classList.add('done');l.classList.add('done');c.innerHTML='<i class="ri-check-line"></i>';}
                    else if(i===n){c.classList.add('active');l.classList.add('active');c.innerHTML=`<i class="${icons[i-1]}"></i>`;}
                    else{c.innerHTML=`<i class="${icons[i-1]}"></i>`;}
                    if(i<4) document.getElementById('wline'+i).classList.toggle('done',i<n);
                }
            }
            ...
            <!-- ══════════════════════════════════════
                 UPDATE PROFILE
            ══════════════════════════════════════ -->
            <div style="background:linear-gradient(135deg,#0d1b2a 0%,#1a2744 60%,#1e2f52 100%);padding:2rem 2rem 0;position:relative;overflow:hidden;">
                <div style="position:absolute;top:-60px;right:-40px;width:220px;height:220px;background:rgba(21,101,192,0.06);border-radius:50%;"></div>
                <div style="display:flex;align-items:flex-end;gap:1.5rem;position:relative;z-index:1;">
                    <div style="position:relative;flex-shrink:0;">
                        <div id="avatarCircle" onclick="document.getElementById('avatarOptions').classList.toggle('hidden')" style="width:90px;height:90px;border-radius:50%;background:linear-gradient(135deg,#1565C0,#00B4D8);display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:800;color:white;border:4px solid rgba(255,255,255,0.1);box-shadow:0 8px 32px rgba(21,101,192,0.4);cursor:pointer;transition:all 0.2s;" title="Click to change color">
                            <?php echo $initials; ?>
                        </div>
                        <div style="position:absolute;bottom:2px;right:2px;width:18px;height:18px;background:var(--cyan);border-radius:50%;border:2px solid #0d1b2a;box-shadow:0 0 8px rgba(0,180,216,0.6);"></div>
                        <div id="avatarOptions" class="hidden" style="position:absolute;top:100%;left:0;margin-top:8px;background:rgba(10,20,40,0.95);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:0.75rem;display:flex;gap:6px;z-index:10;box-shadow:0 8px 32px rgba(0,0,0,0.5);">
                    ...
                    <div style="padding-bottom:1.25rem;flex:1;">
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                            <h2 style="font-size:1.5rem;font-weight:800;color:white;"><?php echo htmlspecialchars($urow['fullname']??'Student'); ?></h2>
                            <span style="background:rgba(0,180,216,0.15);border:1px solid rgba(0,180,216,0.3);border-radius:50px;padding:2px 12px;font-size:0.68rem;font-weight:700;color:#67e8f9;">● Active</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:1rem;margin-top:6px;flex-wrap:wrap;">
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="ri-user-3-line" style="color:var(--cyan)"></i>Student</span>
                            <?php if(!empty($urow['assignments'])): ?><span style="font-size:0.8rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="ri-book-line" style="color:#60a5fa"></i><?php echo htmlspecialchars($urow['assignments']); ?></span><?php endif; ?>
                            <?php if(!empty($urow['join_date'])): ?><span style="font-size:0.8rem;color:rgba(255,255,255,0.35);display:flex;align-items:center;gap:5px;"><i class="ri-calendar-line" style="color:#fbbf24"></i>Joined <?php echo date('M Y',strtotime($urow['join_date'])); ?></span><?php endif; ?>
                        </div>
                    </div>
                    <div style="padding-bottom:1.25rem;">
                        <button onclick="document.getElementById('profileForm').scrollIntoView({behavior:'smooth'})" style="background:rgba(21,101,192,0.15);border:1px solid rgba(0,180,216,0.3);border-radius:9px;padding:7px 14px;color:#67e8f9;font-size:0.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.2s;">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:0.75rem;padding:1rem 1.5rem;border-bottom:1px solid rgba(255,255,255,0.06);">
                <div class="stat-card blue"><div><div class="stat-label">Role</div><div class="stat-value" style="font-size:0.85rem;margin-top:4px;line-height:1.3;">Student</div><div class="stat-sub">Community member</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1565C0,#00B4D8)"><i class="ri-user-3-line"></i></div></div>
                <div class="stat-card cyan"><div><div class="stat-label">Course</div><div class="stat-value" style="font-size:0.72rem;margin-top:4px;line-height:1.4;"><?php echo htmlspecialchars(substr($urow['assignments']??'Not set',0,22)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#00B4D8,#0891b2)"><i class="ri-book-line"></i></div></div>
                <div class="stat-card purple"><div><div class="stat-label">Email</div><div class="stat-value" style="font-size:0.68rem;margin-top:4px;line-height:1.4;word-break:break-all;"><?php echo htmlspecialchars(substr($urow['email']??'Not set',0,24)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#7B2FBE,#6d24a8)"><i class="ri-mail-line"></i></div></div>
                <div class="stat-card yellow"><div><div class="stat-label">Phone</div><div class="stat-value" style="font-size:0.8rem;margin-top:4px;"><?php echo htmlspecialchars($urow['phone']??'Not set'); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#F5A623,#e08c0a)"><i class="ri-phone-line"></i></div></div>
            </div>
            ...
            <div class="content-area" id="profileForm">
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:1.25rem;padding-bottom:0.75rem;border-bottom:1px solid rgba(255,255,255,0.07);">
                    <div style="width:32px;height:32px;border-radius:8px;background:rgba(21,101,192,0.15);border:1px solid rgba(0,180,216,0.25);display:flex;align-items:center;justify-content:center;"><i class="ri-edit-2-line" style="color:var(--cyan);font-size:0.95rem;"></i></div>
                    <div>
                ...
                <form id="profileFormEl" action="student_process.php" method="POST" style="max-width:680px;">
                    ...
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:1.25rem;margin-bottom:1rem;">
                        <div style="font-size:0.68rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:rgba(255,255,255,0.3);margin-bottom:1rem;display:flex;align-items:center;gap:6px;"><i class="ri-information-line" style="color:var(--cyan)"></i>Basic Information</div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-group">
                                <label class="form-label"><i class="ri-user-line" style="color:var(--cyan);margin-right:4px;"></i>Full Name</label>
                                <input type="text" name="fullname" id="inp_fullname" class="form-input" value="<?php echo htmlspecialchars($urow['fullname']??''); ?>" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="ri-mail-line" style="color:#60a5fa;margin-right:4px;"></i>Email Address</label>
                                <input type="email" name="email" id="inp_email" class="form-input" value="<?php echo htmlspecialchars($urow['email']??''); ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="ri-phone-line" style="color:#fbbf24;margin-right:4px;"></i>Phone Number</label>
                                <input type="text" name="phone" id="inp_phone" class="form-input" value="<?php echo htmlspecialchars($urow['phone']??''); ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label"><i class="ri-user-3-line" style="color:#c084fc;margin-right:4px;"></i>Role</label>
                                <input type="text" class="form-input" value="Student" disabled style="opacity:0.4;">
                            </div>
                            <div class="form-group" style="grid-column:1/-1;">
                                <label class="form-label"><i class="ri-book-line" style="color:var(--cyan);margin-right:4px;"></i>Course / Year Level</label>
                                <input type="text" name="assignments" id="inp_assignments" class="form-input" value="<?php echo htmlspecialchars($urow['assignments']??''); ?>">
                            </div>
                        </div>
                    </div>
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:1.25rem;margin-bottom:1.25rem;">
                        ...
                        <div id="pwStrengthWrap" style="display:none;margin-top:0.5rem;">
                            <div style="display:flex;align-items:center;gap:8px;">
                                <div style="flex:1;height:4px;background:rgba(255,255,255,0.1);border-radius:4px;overflow:hidden;"><div id="pwStrengthBar" style="height:100%;width:0%;border-radius:4px;transition:all 0.3s;"></div></div>
            ...
            <!-- Save Confirm Modal -->
            <div id="saveConfirmModal" style="display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.75);backdrop-filter:blur(6px);align-items:center;justify-content:center;">
                <div style="background:linear-gradient(135deg,#0d1b2a,#0a1424);border:1px solid rgba(21,101,192,0.2);border-radius:18px;padding:2rem;width:100%;max-width:420px;margin:1rem;box-shadow:0 24px 64px rgba(0,0,0,0.6);text-align:center;">
                    <div style="width:64px;height:64px;border-radius:50%;background:rgba(21,101,192,0.15);border:1px solid rgba(0,180,216,0.3);display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;font-size:1.8rem;color:var(--cyan);"><i class="ri-save-3-line"></i></div>
                    ...
                    <div style="display:flex;gap:0.75rem;">
                        ...
                        <button onclick="submitProfileForm()" style="flex:1;padding:0.7rem;border-radius:10px;background:linear-gradient(135deg,#1565C0,#00B4D8);border:none;color:white;font-weight:700;cursor:pointer;font-family:inherit;font-size:0.85rem;box-shadow:0 4px 14px rgba(21,101,192,0.4);"><i class="ri-check-line"></i> Yes, Save</button>
                    </div>
                </div>
            </div>
            ...
            function openSaveConfirm(){
                ...
                p+=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Course:</span><span style="color:white;font-weight:600;">${dept||'—'}</span></div>`;
            ...
            <div class="content-area">
                <div id="locationsGrid"><div class="text-center py-12"><i class="ri-loader-4-line text-3xl animate-spin" style="color:var(--cyan)"></i><p class="text-sm mt-3" style="color:rgba(255,255,255,0.35)">Loading locations...</p></div></div>
            </div>

            <?php elseif($section === 'activity'): ?>
            <div class="stat-grid">
                <div class="stat-card blue"><div><div class="stat-label">Total</div><div class="stat-value" id="statTotal">0</div><div class="stat-sub">All registered</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1565C0,#1976D2)"><i class="ri-calendar-line"></i></div></div>
                <div class="stat-card cyan"><div><div class="stat-label">Ongoing</div><div class="stat-value" id="statOngoing">0</div><div class="stat-sub" style="color:#67e8f9">• In progress</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#00B4D8,#0891b2)"><i class="ri-play-circle-line"></i></div></div>
                <div class="stat-card blue"><div><div class="stat-label">Upcoming</div><div class="stat-value" id="statUpcoming">0</div><div class="stat-sub">Scheduled</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#7B2FBE,#6d24a8)"><i class="ri-calendar-check-line"></i></div></div>
            ...
            <div class="content-area">
                ...
                <div id="activity-view">
                    ...
                    <div id="activitiesContent"><div class="text-center py-8"><i class="ri-loader-4-line animate-spin" style="color:var(--cyan);font-size:1.5rem"></i></div></div>
                </div>
                <div id="activity-feedback" class="hidden" style="max-width:520px;">
                    ...
                    <form method="POST" action="student_process.php" class="space-y-4">
                        ...
                        <div class="form-group"><label class="form-label">Your Feedback</label><textarea name="feedback" rows="4" class="form-input" placeholder="Share your thoughts..." style="resize:none;"></textarea></div>
            ...
            <div class="stat-grid">
                <div class="stat-card blue"><div><div class="stat-label">Total</div><div class="stat-value" id="pStatTotal">0</div><div class="stat-sub">All projects</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#7B2FBE,#6d24a8)"><i class="ri-folder-2-line"></i></div></div>
                <div class="stat-card cyan"><div><div class="stat-label">Active</div><div class="stat-value" id="pStatActive">0</div><div class="stat-sub" style="color:#67e8f9">• In progress</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1DB954,#16a34a)"><i class="ri-play-circle-line"></i></div></div>
                <div class="stat-card blue"><div><div class="stat-label">Planning</div><div class="stat-value" id="pStatPlanning">0</div><div class="stat-sub">Scheduled</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1565C0,#1976D2)"><i class="ri-calendar-check-line"></i></div></div>
            ...
            <div class="content-area">
                ...
                <div id="project-view">
                    ...
                    <div id="projectsContent"><div class="text-center py-8"><i class="ri-loader-4-line animate-spin" style="color:var(--cyan);font-size:1.5rem"></i></div></div>
                </div>
            ...
            <div class="content-area">
                <?php if(empty($evals)): ?>
                <div class="text-center py-16"><i class="ri-feedback-line text-5xl" style="color:rgba(255,255,255,0.08)"></i><p class="mt-4 text-sm" style="color:rgba(255,255,255,0.35)">No feedback submitted yet.</p><p class="text-xs mt-1" style="color:rgba(255,255,255,0.2)">Go to Activities or Projects to submit feedback.</p></div>
                <?php else: ?>
        ...
        <div class="status-bar"><span>© <?php echo date('Y'); ?> CEMS. All rights reserved.</span><span>Student Portal &nbsp;•&nbsp; v2.0</span></div>
    </div>
</main>
...
<div class="modal-overlay" id="joinModal">
    <div class="modal-box">
        <div class="modal-icon-wrap" id="modalIconWrap"><i id="modalIcon" class="ri-user-add-line"></i></div>
        <div class="modal-title" id="modalTitle">Join</div>
        <div class="modal-msg">Are you sure you want to join<br><span class="modal-name" id="modalName"></span>?</div>
        <div class="modal-btns">
            <button class="modal-btn-cancel" onclick="closeJoinModal()"><i class="ri-close-line"></i> Cancel</button>
            <button class="modal-btn-confirm" id="modalConfirmBtn" onclick="confirmJoin()"><i class="ri-check-line"></i> Join Now</button>
        </div>
    </div>
</div>
...
document.querySelectorAll('a.nav-item:not(.locked)').forEach(link=>{
    link.addEventListener('click',e=>{
        if(link.href.includes('student.php')&&!link.href.includes('logout')){
            e.preventDefault();
...
function showSetupAlert(){ showToast('Please complete your profile first to access this section!','error'); }
...
function showToast(msg,type='success'){
    ...
    icon.style.color=type==='success'?'#67e8f9':'#f87171';
    setTimeout(()=>toast.classList.add('show'),10);
    setTimeout(()=>toast.classList.remove('show'),3500);
}

let _joinType='',_joinId=0;
function joinActivity(id,name){ _joinType='activity'; _joinId=id; const w=document.getElementById('modalIconWrap'),ic=document.getElementById('modalIcon'); w.style.background='rgba(0,180,216,0.15)'; w.style.border='1px solid rgba(0,180,216,0.3)'; ic.className='ri-calendar-event-line'; ic.style.color='#00B4D8'; document.getElementById('modalTitle').textContent='Join Activity'; document.getElementById('modalName').textContent='"'+name+'"'; const btn=document.getElementById('modalConfirmBtn'); btn.style.background='linear-gradient(135deg,#1565C0,#00B4D8)'; btn.innerHTML='<i class="ri-calendar-check-line mr-1"></i>Join Activity'; openJoinModal(); }
function joinProject(id,name){ _joinType='project'; _joinId=id; const w=document.getElementById('modalIconWrap'),ic=document.getElementById('modalIcon'); w.style.background='rgba(123,47,190,0.15)'; w.style.border='1px solid rgba(123,47,190,0.3)'; ic.className='ri-folder-add-line'; ic.style.color='#a855f7'; document.getElementById('modalTitle').textContent='Join Project'; document.getElementById('modalName').textContent='"'+name+'"'; const btn=document.getElementById('modalConfirmBtn'); btn.style.background='linear-gradient(135deg,#7B2FBE,#1565C0)'; btn.innerHTML='<i class="ri-folder-add-line mr-1"></i>Join Project'; openJoinModal(); }
function openJoinModal(){ document.getElementById('joinModal').classList.add('show'); document.body.style.overflow='hidden'; }
function closeJoinModal(){ document.getElementById('joinModal').classList.remove('show'); document.body.style.overflow=''; }
function confirmJoin(){
    if(!_joinId) return;
    const btn=document.getElementById('modalConfirmBtn'); btn.classList.add('loading'); btn.textContent='Joining...';
    const fd=new FormData(); fd.append('action',_joinType==='activity'?'join_activity':'join_project'); fd.append(_joinType==='activity'?'activity_id':'project_id',_joinId);
    fetch('student_process.php',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{ closeJoinModal(); if(d.status==='success') showToast(d.message||'Joined successfully! 🎉','success'); else showToast(d.message||'Something went wrong.','error'); }).catch(()=>{ closeJoinModal(); showToast('Network error. Please try again.','error'); }).finally(()=>{ btn.classList.remove('loading'); });
}
...
function renderActivities(data){
    ...
    const sc={'Upcoming':'badge-blue','Ongoing':'badge-cyan','Completed':'badge-gray','Cancelled':'badge-gray'};
    document.getElementById('activitiesContent').innerHTML=`<div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Activity</th><th>Date / Location</th><th>Status</th><th>Action</th></tr></thead><tbody>${data.map(a=>`<tr><td><div class="flex items-center gap-3"><div class="row-icon" style="background:linear-gradient(135deg,#1565C0,#00B4D8)"><i class="ri-calendar-event-line"></i></div><div class="row-name">${a.name||'—'}</div></div></td><td class="row-sub">${a.date_time||'—'} ${a.location?'• '+a.location:''}</td><td><span class="badge ${sc[a.status]||'badge-gray'}">${a.status||'—'}</span></td><td><button class="join-btn" onclick="joinActivity(${a.id},'${(a.name||'').replace(/'/g,"\\'")}')">+ Join</button></td></tr>`).join('')}</tbody></table><div class="mt-3 text-xs" style="color:rgba(255,255,255,0.3)">Total: <strong style="color:white">${data.length}</strong> activities</div></div>`;
}
...
function renderProjects(data){
    ...
    const sc={'Active':'badge-cyan','Planning':'badge-blue','Completed':'badge-gray','On Hold':'badge-yellow'};
    document.getElementById('projectsContent').innerHTML=`<div class="overflow-x-auto"><table class="data-table"><thead><tr><th>Project</th><th>Timeline</th><th>Status</th><th>Action</th></tr></thead><tbody>${data.map(p=>`<tr><td><div class="flex items-center gap-3"><div class="row-icon" style="background:linear-gradient(135deg,#7B2FBE,#1565C0)"><i class="ri-folder-2-line"></i></div><div class="row-name">${p.name||'—'}</div></div></td><td class="row-sub">${p.start_date||'—'}</td><td><span class="badge ${sc[p.status]||'badge-gray'}">${p.status||'—'}</span></td><td><button class="join-btn" onclick="joinProject(${p.id},'${(p.name||'').replace(/'/g,"\\'")}')">+ Join</button></td></tr>`).join('')}</tbody></table><div class="mt-3 text-xs" style="color:rgba(255,255,255,0.3)">Total: <strong style="color:white">${data.length}</strong> projects</div></div>`;
}

...
<title>CEMS - Teaching Staff Portal</title>
...
.sidebar-brand-icon { width:28px; height:28px; border-radius:7px; background:linear-gradient(135deg,var(--green),var(--teal)); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
...
.sidebar-user { background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:0.6rem; margin-bottom:1.2rem; display:flex; flex-direction:column; align-items:center; gap:4px; }
.sidebar-avatar { width:36px; height:36px; border-radius:9px; background:linear-gradient(135deg,var(--green),var(--teal)); display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.95rem; color:white; }
...
.nav-item.active { background:rgba(29,185,84,0.18); border-color:rgba(29,185,84,0.35); color:white; font-weight:700; box-shadow:0 0 12px rgba(29,185,84,0.15); }
...
.nav-item.locked:hover i { color:rgba(255,255,255,0.4); }
...
.mac-topbar { background:rgba(10,20,15,0.7); backdrop-filter:blur(30px); border-bottom:1px solid rgba(29,185,84,0.1); padding:0.6rem 1.5rem; display:flex; align-items:center; justify-content:space-between; position:sticky; top:0; z-index:30; }
...
.refresh-btn { background:rgba(29,185,84,0.12); border:1px solid rgba(29,185,84,0.25); border-radius:8px; padding:6px 14px; color:white; font-size:0.75rem; font-weight:600; cursor:pointer; display:flex; align-items:center; gap:5px; transition:all 0.2s; }
...
.glass-card { background:rgba(30,30,40,0.6); backdrop-filter:blur(20px); border:1px solid rgba(255,255,255,0.08); border-radius:14px; overflow:hidden; }
.section-header { background:linear-gradient(135deg,#212529 0%,#343A40 50%,#495057 100%); padding:1.25rem 1.5rem; position:relative; overflow:hidden; }
.section-header::before { content:''; position:absolute; top:-80px; right:-40px; width:220px; height:220px; background:rgba(255,255,255,0.04); border-radius:50%; }
.section-header-badge { display:inline-flex; align-items:center; gap:5px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.15); border-radius:50px; padding:3px 10px; font-size:0.65rem; font-weight:700; color:rgba(255,255,255,0.7); letter-spacing:0.08em; text-transform:uppercase; margin-bottom:0.5rem; }
...
.stat-card.pink::before { background:rgba(233,30,140,0.15); }
...
.search-input { flex:1; background:rgba(0,0,0,0.25); border:1px solid rgba(255,255,255,0.08); border-radius:8px; padding:0.55rem 0.9rem 0.55rem 2.2rem; font-size:0.8rem; color:white; outline:none; font-family:inherit; transition:all 0.2s; }
...
.badge-pink   { background:rgba(233,30,140,0.15); color:#f472b6; border:1px solid rgba(233,30,140,0.3); }
.join-btn { padding:0.35rem 0.85rem; border-radius:7px; font-size:0.72rem; font-weight:700; cursor:pointer; transition:all 0.2s; background:rgba(29,185,84,0.12); border:1px solid rgba(29,185,84,0.3); color:#4ade80; font-family:inherit; }
...
.form-input { width:100%; background:rgba(0,0,0,0.25); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:0.7rem 1rem; font-size:0.85rem; color:white; outline:none; transition:all 0.2s; font-family:inherit; }
...
.btn-save { background:linear-gradient(135deg,var(--green),var(--green2)); color:white; border:none; border-radius:9px; padding:0.7rem 1.8rem; font-size:0.875rem; font-weight:700; cursor:pointer; transition:all 0.2s; font-family:inherit; box-shadow:0 4px 14px rgba(29,185,84,0.3); display:inline-flex; align-items:center; gap:6px; }
...
.modal-overlay { position:fixed; inset:0; z-index:999; background:rgba(0,0,0,0.7); backdrop-filter:blur(6px); display:flex; align-items:center; justify-content:center; opacity:0; pointer-events:none; transition:opacity 0.2s; }
.modal-overlay.show { opacity:1; pointer-events:all; }
.modal-box { background:linear-gradient(135deg,#1e2228 0%,#16191e 100%); border:1px solid rgba(255,255,255,0.1); border-radius:18px; padding:2rem; width:100%; max-width:400px; margin:1rem; box-shadow:0 24px 64px rgba(0,0,0,0.5); transform:scale(0.95) translateY(10px); transition:all 0.25s cubic-bezier(0.175,0.885,0.32,1.275); text-align:center; }
...
.modal-btn-confirm { flex:1; padding:0.7rem 1rem; border-radius:10px; font-size:0.85rem; font-weight:700; cursor:pointer; font-family:inherit; border:none; color:white; transition:all 0.2s; box-shadow:0 4px 14px rgba(29,185,84,0.3); }
.modal-btn-confirm:hover { transform:translateY(-1px); }
.modal-btn-confirm.loading { opacity:0.7; pointer-events:none; }
.toast { position:fixed; bottom:1.5rem; right:1.5rem; z-index:9999; background:linear-gradient(135deg,#1e2228,#16191e); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:0.85rem 1.25rem; display:flex; align-items:center; gap:10px; font-size:0.85rem; font-weight:600; color:white; box-shadow:0 8px 32px rgba(0,0,0,0.4); transform:translateY(20px); opacity:0; transition:all 0.3s cubic-bezier(0.175,0.885,0.32,1.275); min-width:260px; }
...
.mobile-topbar { position:fixed; top:0; left:0; right:0; z-index:50; background:rgba(10,20,15,0.9); backdrop-filter:blur(20px); border-bottom:1px solid rgba(29,185,84,0.1); padding:0.75rem 1rem; display:flex; align-items:center; justify-content:space-between; }
.setup-banner { background:linear-gradient(135deg,rgba(245,166,35,0.15),rgba(245,166,35,0.08)); border:1px solid rgba(245,166,35,0.3); border-radius:12px; padding:1rem 1.25rem; margin-bottom:1.25rem; display:flex; align-items:center; gap:12px; }
.setup-banner i { font-size:1.5rem; color:#fbbf24; flex-shrink:0; }
.setup-banner-text { flex:1; }
.setup-banner-title { font-size:0.9rem; font-weight:700; color:#fbbf24; }
.setup-banner-desc { font-size:0.78rem; color:rgba(255,255,255,0.5); margin-top:2px; }
/* ── Wizard styles ── */
...
.wstep { display:flex; flex-direction:column; align-items:center; gap:6px; position:relative; }
.wstep-circle { width:42px; height:42px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.85rem; font-weight:800; border:2px solid rgba(255,255,255,0.12); background:rgba(255,255,255,0.04); color:rgba(255,255,255,0.3); transition:all 0.4s ease; position:relative; z-index:1; }
.wstep-circle.active { background:linear-gradient(135deg,#1DB954,#16a34a); border-color:#1DB954; color:white; box-shadow:0 0 20px rgba(29,185,84,0.5); }
.wstep-circle.done   { background:rgba(29,185,84,0.2); border-color:rgba(29,185,84,0.5); color:#4ade80; }
...
.wstep-label.active { color:#4ade80; }
.wstep-label.done   { color:rgba(29,185,84,0.6); }
...
.wstep-line.done { background:rgba(29,185,84,0.4); }
...
.field-input { width:100%; background:rgba(0,0,0,0.3); border:1.5px solid rgba(255,255,255,0.08); border-radius:11px; padding:0.75rem 1rem; font-size:0.875rem; color:white; outline:none; transition:all 0.2s; font-family:inherit; }
.field-input::placeholder { color:rgba(255,255,255,0.18); }
.field-input:focus { border-color:#1DB954; box-shadow:0 0 0 3px rgba(29,185,84,0.12); background:rgba(0,0,0,0.4); }
...
.wbtn-next { padding:0.65rem 1.6rem; border-radius:10px; background:linear-gradient(135deg,#1DB954,#16a34a); border:none; color:white; font-size:0.85rem; font-weight:800; cursor:pointer; font-family:inherit; transition:all 0.2s; display:flex; align-items:center; gap:6px; box-shadow:0 4px 16px rgba(29,185,84,0.35); }
.wbtn-next:hover { transform:translateY(-1px); box-shadow:0 6px 22px rgba(29,185,84,0.45); }
...
.avatar-swatch.chosen { border-color:white; box-shadow:0 0 0 3px rgba(29,185,84,0.5); transform:scale(1.1); }
...
<aside class="mac-sidebar hidden md:flex flex-col">
    <div class="sidebar-brand">
        ...
        <div><div class="sidebar-brand-text">CEMS</div><div class="sidebar-brand-sub">Staff Portal</div></div>
    </div>
    ...
    <a href="teaching_staff.php?section=profile" class="nav-item <?php echo $section==='profile'?'active':''; ?>">
    ...
    <a href="<?php echo $profileIncomplete?'#':'teaching_staff.php?section=location'; ?>" class="nav-item <?php echo $section==='location'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-map-pin-line"></i><span>Locations</span>
    </a>
    <a href="<?php echo $profileIncomplete?'#':'teaching_staff.php?section=activity'; ?>" class="nav-item <?php echo $section==='activity'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-calendar-event-line"></i><span>Activities</span>
    </a>
    <a href="<?php echo $profileIncomplete?'#':'teaching_staff.php?section=project'; ?>" class="nav-item <?php echo $section==='project'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-folder-2-line"></i><span>Projects</span>
    </a>
    <div class="nav-divider"></div>
    <span class="nav-label">Reports</span>
    <a href="<?php echo $profileIncomplete?'#':'teaching_staff.php?section=feedback'; ?>" class="nav-item <?php echo $section==='feedback'?'active':''; ?> <?php echo $profileIncomplete?'locked':''; ?>" <?php if($profileIncomplete) echo 'onclick="showSetupAlert();return false;"'; ?>>
        <i class="ri-feedback-line"></i><span>Feedback</span>
    </a>
    <div style="flex:1"></div>
    <a href="teaching_staff.php?section=logout" class="logout-btn"><i class="ri-logout-box-line"></i><span>Logout</span></a>
...
<div id="mobileSidebar" class="md:hidden hidden fixed inset-0 z-50">
    ...
    <div class="mac-sidebar absolute" style="width:148px;">
        ...
        <a href="teaching_staff.php?section=profile" class="nav-item <?php echo $section==='profile'?'active':''; ?>"><i class="ri-user-3-line"></i><span>Profile</span><?php if($profileIncomplete): ?><span class="nav-badge"></span><?php endif; ?></a>
        <div class="nav-divider"></div>
        <a href="teaching_staff.php?section=location" class="nav-item <?php echo $section==='location'?'active':''; ?>"><i class="ri-map-pin-line"></i><span>Locations</span></a>
        <a href="teaching_staff.php?section=activity" class="nav-item <?php echo $section==='activity'?'active':''; ?>"><i class="ri-calendar-event-line"></i><span>Activities</span></a>
        <a href="teaching_staff.php?section=project"  class="nav-item <?php echo $section==='project'?'active':''; ?>"><i class="ri-folder-2-line"></i><span>Projects</span></a>
        <div class="nav-divider"></div>
        <a href="teaching_staff.php?section=feedback" class="nav-item <?php echo $section==='feedback'?'active':''; ?>"><i class="ri-feedback-line"></i><span>Feedback</span></a>
...
<main class="md:ml-[148px] min-h-screen flex flex-col">
    <div class="mac-topbar hidden md:flex">
        ...
        <div class="flex items-center gap-3">
            <span class="sys-status"><span class="w-2 h-2 rounded-full animate-pulse" style="background:var(--green);box-shadow:0 0 6px var(--green)"></span>System Online</span>
    ...
    <div class="flex-1 p-4 md:p-6 mt-14 md:mt-0 fade-in">
        ...
        <?php
        $pageInfo = [
            'profile'  => ['My Profile',    'PROFILE',    'ri-user-3-line',         $profileIncomplete ? 'Complete your profile to unlock all features' : 'View and update your personal profile'],
        ...
        <div class="glass-card">
            <div class="section-header">
                ...
                <div class="section-desc">
                    <i class="ri-sparkling-line" style="color:var(--green)"></i><?php echo $pi[3]; ?>
                </div>
            </div>
            ...
            $swatches = [['#1DB954','#16a34a'],['#1565C0','#1976D2'],['#8B2FC9','#6d24a8'],['#E91E8C','#c4176f'],['#F5A623','#e08c0a'],['#E32636','#b81c28'],['#00B4A0','#00897B'],['#FF6B35','#e55a24']];
            ?>
            ...
            <!-- Hidden form that gets submitted on step 4 -->
            <form id="createProfileForm" action="teaching_staff_process.php" method="POST">
            ...
            <div class="wizard-wrap">
                ...
                <div class="wizard-steps">
                    <div class="wstep">
                        <div class="wstep-circle active" id="ws1"><i class="ri-user-line"></i></div>
                        <div class="wstep-label active"  id="wl1">Identity</div>
                    </div>
                    <div class="wstep-line" id="wline1"></div>
                    <div class="wstep">
                        <div class="wstep-circle" id="ws2"><i class="ri-building-line"></i></div>
                        <div class="wstep-label"  id="wl2">Department</div>
                    </div>
                    <div class="wstep-line" id="wline2"></div>
                    <div class="wstep">
                        <div class="wstep-circle" id="ws3"><i class="ri-lock-line"></i></div>
                        <div class="wstep-label"  id="wl3">Security</div>
                    </div>
                    <div class="wstep-line" id="wline3"></div>
                    <div class="wstep">
                        <div class="wstep-circle" id="ws4"><i class="ri-check-line"></i></div>
                        <div class="wstep-label"  id="wl4">Review</div>
                    </div>
                </div>
                ...
                <div class="wizard-panel active" id="wpanel1">
                    ...
                    <div class="wizard-panel-sub">Tell us who you are — this is how others will see you in the system.</div>

                    <div style="display:flex;align-items:flex-start;gap:1.5rem;margin-bottom:1.75rem;flex-wrap:wrap;">
                        <div style="display:flex;flex-direction:column;align-items:center;gap:0.75rem;">
                            <div class="avatar-preview" id="avatarPreview" style="background:linear-gradient(135deg,#1DB954,#16a34a)">
                            ...
                            <div class="avatar-picker">
                                <?php foreach([['#1DB954','#16a34a'],['#1565C0','#1976D2'],['#8B2FC9','#6d24a8'],['#E91E8C','#c4176f'],['#F5A623','#e08c0a'],['#00B4A0','#00897B'],['#E32636','#b81c28'],['#FF6B35','#e55a24']] as $i=>$sw): ?>
                        ...
                        <div style="flex:1;min-width:220px;">
                            <div class="field-group">
                                <label class="field-label"><i class="ri-user-line" style="color:#1DB954;margin-right:4px;"></i>Full Name <span style="color:#f87171;">*</span></label>
                ...
                <div class="wizard-panel" id="wpanel2">
                    <div class="wizard-panel-title">🏫 Your Department</div>
                    <div class="wizard-panel-sub">Where are you assigned? This helps coordinators match you to the right programs.</div>
                    <div class="field-group">
                        <label class="field-label"><i class="ri-building-line" style="color:#60a5fa;margin-right:4px;"></i>Department / Assignment <span style="color:#f87171;">*</span></label>
                        <input type="text" id="cp_assignments" class="field-input" placeholder="e.g., College of Education, SHS Department">
                        <div class="field-error" id="err_assignments">Please enter your department.</div>
                        <div class="field-hint"><i class="ri-information-line"></i>You can update this later from your profile</div>
                    </div>
                    <div style="margin-top:0.5rem;">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:6px;">
                            <?php foreach(['College of Education','College of Engineering','College of Arts & Sciences','Senior High School','Basic Education','College of Business','College of Nursing','College of IT'] as $d): ?>
                            <button type="button" onclick="document.getElementById('cp_assignments').value='<?php echo $d; ?>'"
                                    ...
                                    onmouseover="this.style.background='rgba(29,185,84,0.15)';this.style.borderColor='rgba(29,185,84,0.3)';this.style.color='#4ade80'"
                                    onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.borderColor='rgba(255,255,255,0.1)';this.style.color='rgba(255,255,255,0.5)'"><?php echo $d; ?></button>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                ...
                <div class="wizard-panel" id="wpanel3">
                    ...
                    <div class="wizard-panel-sub">Leave blank to keep your current password. We recommend setting a strong one.</div>
                    ...
                    <div style="margin-top:0.75rem;padding:0.75rem 1rem;background:rgba(245,166,35,0.06);border:1px solid rgba(245,166,35,0.2);border-radius:10px;display:flex;align-items:center;gap:8px;">
                        ...
                        <span style="font-size:0.75rem;color:rgba(255,255,255,0.45);">You can skip password setup and use your existing one to log in.</span>
                    </div>
                    <div class="wizard-nav">
                ...
                <div class="wizard-panel" id="wpanel4">
                    ...
                    <div class="wizard-panel-sub">Everything look good? Hit "Create Profile" to unlock all features.</div>
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:1.25rem;margin-bottom:1.5rem;">
                        <div style="display:flex;align-items:center;gap:14px;margin-bottom:1.25rem;padding-bottom:1rem;border-bottom:1px solid rgba(255,255,255,0.06);">
                            <div id="reviewAvatar" style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#1DB954,#16a34a);display:flex;align-items:center;justify-content:center;font-size:1.4rem;font-weight:800;color:white;border:3px solid rgba(255,255,255,0.1);flex-shrink:0;"></div>
                            <div>
                                <div id="reviewName" style="font-size:1.1rem;font-weight:800;color:white;"></div>
                                <div style="font-size:0.75rem;color:rgba(255,255,255,0.4);margin-top:2px;display:flex;align-items:center;gap:5px;"><i class="ri-shield-user-line" style="color:#1DB954"></i>Teaching Staff</div>
                            </div>
                        ...
                        <div class="review-row"><div class="review-icon" style="background:rgba(139,47,201,0.15);"><i class="ri-building-line" style="color:#c084fc;"></i></div><div class="review-key">Department</div><div class="review-val" id="rv_dept">—</div></div>
                    ...
                    <div style="background:linear-gradient(135deg,rgba(29,185,84,0.08),rgba(0,180,160,0.06));border:1px solid rgba(29,185,84,0.2);border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.5rem;">
                        <div style="font-size:0.72rem;font-weight:800;text-transform:uppercase;letter-spacing:0.1em;color:#4ade80;margin-bottom:0.75rem;display:flex;align-items:center;gap:6px;"><i class="ri-unlock-line"></i>Features you'll unlock</div>
                        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
                            <?php foreach(['📍 View Locations','📅 View Activities','✋ Join Activities','📁 View Projects','🤝 Join Projects','💬 Submit Feedback'] as $f): ?>
                            <div style="font-size:0.78rem;color:rgba(255,255,255,0.6);display:flex;align-items:center;gap:6px;"><span style="color:#4ade80;">✓</span><?php echo $f; ?></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="wizard-nav">
                        ...
                        <button type="button" class="wbtn-next" id="createBtn" onclick="submitCreateProfile()"
                                style="background:linear-gradient(135deg,#1DB954,#00B4A0);padding-left:2rem;padding-right:2rem;">
            ...
            let chosenC1 = '#1DB954', chosenC2 = '#16a34a';

            function pickAvatarColor(el) {
                document.querySelectorAll('.avatar-swatch').forEach(s => s.classList.remove('chosen'));
                el.classList.add('chosen');
                chosenC1 = el.dataset.c1; chosenC2 = el.dataset.c2;
                document.getElementById('avatarPreview').style.background = `linear-gradient(135deg,${chosenC1},${chosenC2})`;
            ...
            function updateInitials(val) {
                const init = val.trim().split(' ').filter(Boolean).slice(0,2).map(w=>w[0].toUpperCase()).join('');
            ...
            function checkPwStrength(v) {
                ...
                const lvl=[{w:'20%',c:'#ef4444',l:'Weak'},{w:'40%',c:'#f97316',l:'Fair'},{w:'65%',c:'#fbbf24',l:'Good'},{w:'85%',c:'#86efac',l:'Strong'},{w:'100%',c:'#4ade80',l:'Very Strong'}][Math.min(s,4)];
                bar.style.width=lvl.w; bar.style.background=lvl.c; lbl.textContent=lvl.l; lbl.style.color=lvl.c;
            }

            function toggleWizPw(id,btn) {
                const i=document.getElementById(id);
                if(i.type==='password'){i.type='text';btn.innerHTML='<i class="ri-eye-off-line"></i>';}
                else{i.type='password';btn.innerHTML='<i class="ri-eye-line"></i>';}
            }
            ...
            function validateStep(step) {
                let ok=true;
                const show=(id,s)=>{
                    const el=document.getElementById(id); if(el) el.style.display=s?'flex':'none';
                    const inp=document.getElementById(id.replace('err_','cp_')); if(inp) inp.classList.toggle('error',s);
                };
                if(step===1){
                    const name=document.getElementById('cp_fullname').value.trim();
                    const phone=document.getElementById('cp_phone').value.trim();
                    show('err_fullname',!name); show('err_phone',!phone);
                    if(!name||!phone) ok=false;
                }
                if(step===2){
                    const dept=document.getElementById('cp_assignments').value.trim();
                    show('err_assignments',!dept); if(!dept) ok=false;
                }
                if(step===3){
                    const pw=document.getElementById('cp_password').value;
                    const cfm=document.getElementById('cp_confirm').value;
                    if(pw!==''&&pw.length<6){show('err_password',true);ok=false;} else show('err_password',false);
                    if(pw!==''&&pw!==cfm){show('err_confirm',true);ok=false;} else show('err_confirm',false);
                }
                return ok;
            }
            ...
            <?php else: ?>
            <!-- ══════════════════════════════════════════════
                 UPDATE PROFILE — existing user editor
            ══════════════════════════════════════════════ -->
            ...
            <div style="background:linear-gradient(135deg,#1a1f2e 0%,#212529 60%,#2d3748 100%);padding:2rem 2rem 0;position:relative;overflow:hidden;">
                ...
                <div style="display:flex;align-items:flex-end;gap:1.5rem;position:relative;z-index:1;">
                    <div style="position:relative;flex-shrink:0;">
                        <div id="avatarCircle" onclick="document.getElementById('avatarOptions').classList.toggle('hidden')" style="width:90px;height:90px;border-radius:50%;background:linear-gradient(135deg,#1DB954,#16a34a);display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:800;color:white;border:4px solid rgba(255,255,255,0.1);box-shadow:0 8px 32px rgba(29,185,84,0.3);cursor:pointer;transition:all 0.2s;" title="Click to change color">
                            <?php echo $initials; ?>
                        </div>
                    ...
                    <div style="padding-bottom:1.25rem;flex:1;">
                        ...
                        <div style="display:flex;align-items:center;gap:1rem;margin-top:6px;flex-wrap:wrap;">
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.45);display:flex;align-items:center;gap:5px;"><i class="ri-shield-user-line" style="color:var(--green)"></i>Teaching Staff</span>
                    ...
                    <div style="padding-bottom:1.25rem;">
                        <button onclick="document.getElementById('profileForm').scrollIntoView({behavior:'smooth'})" style="background:rgba(29,185,84,0.15);border:1px solid rgba(29,185,84,0.3);border-radius:9px;padding:7px 14px;color:#4ade80;font-size:0.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.2s;">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:0.75rem;padding:1rem 1.5rem;border-bottom:1px solid rgba(255,255,255,0.06);">
                <div class="stat-card green"><div><div class="stat-label">Role</div><div class="stat-value" style="font-size:0.85rem;margin-top:4px;line-height:1.3;">Teaching<br>Staff</div><div class="stat-sub">Staff member</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1DB954,#16a34a)"><i class="ri-user-star-line"></i></div></div>
                <div class="stat-card blue"><div><div class="stat-label">Department</div><div class="stat-value" style="font-size:0.72rem;margin-top:4px;line-height:1.4;"><?php echo htmlspecialchars(substr($urow['assignments']??'Not set',0,22)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#1565C0,#1976D2)"><i class="ri-building-line"></i></div></div>
                <div class="stat-card teal"><div><div class="stat-label">Email</div><div class="stat-value" style="font-size:0.68rem;margin-top:4px;line-height:1.4;word-break:break-all;"><?php echo htmlspecialchars(substr($urow['email']??'Not set',0,24)); ?></div></div><div class="stat-icon" style="background:linear-gradient(135deg,#00B4A0,#00897B)"><i class="ri-mail-line"></i></div></div>
            ...
            <div class="content-area" id="profileForm">
                ...
                <form id="profileFormEl" action="teaching_staff_process.php" method="POST" style="max-width:680px;">
                    ...
                    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:1.25rem;margin-bottom:1rem;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-input" value="Teaching Staff" disabled style="opacity:0.4;">
                            </div>
            ...
            <div id="saveConfirmModal" style="display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.7);backdrop-filter:blur(6px);align-items:center;justify-content:center;">
                <div style="background:linear-gradient(135deg,#1e2228,#16191e);border:1px solid rgba(255,255,255,0.1);border-radius:18px;padding:2rem;width:100%;max-width:420px;margin:1rem;box-shadow:0 24px 64px rgba(0,0,0,0.5);text-align:center;">
                    <div style="width:64px;height:64px;border-radius:50%;background:rgba(29,185,84,0.15);border:1px solid rgba(29,185,84,0.3);display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;font-size:1.8rem;color:var(--green);"><i class="ri-save-3-line"></i></div>
            ...
            function openSaveConfirm(){
                ...
                p+=`<div style="margin-bottom:5px;"><span style="color:rgba(255,255,255,0.4);width:80px;display:inline-block;">Dept:</span><span style="color:white;font-weight:600;">${dept||'—'}</span></div>`;
            ...
            <?php endif; // end profileIncomplete ?>

            <?php elseif($section === 'location'): ?>
            <div class="content-area">
                <div id="locationsGrid"><div class="text-center py-12"><i class="ri-loader-4-line text-3xl animate-spin" style="color:var(--green)"></i><p class="text-sm mt-3" style="color:rgba(255,255,255,0.35)">Loading locations...</p></div></div>
            </div>
            ...
            <div class="content-area">
                ...
                <div id="activity-view">
                    ...
                    <div id="activitiesContent"><div class="text-center py-8"><i class="ri-loader-4-line animate-spin" style="color:var(--green);font-size:1.5rem"></i></div></div>
                </div>
                <div id="activity-feedback" class="hidden" style="max-width:520px;">
                    ...
                    <form method="POST" action="teaching_staff_process.php" class="space-y-4">
            ...
            <div class="stat-grid">
                <div class="stat-card blue"><div><div class="stat-label">Total</div><div class="stat-value" id="pStatTotal">0</div><div class="stat-sub">All projects</div></div><div class="stat-icon" style="background:linear-gradient(135deg,#8B2FC9,#6d24a8)"><i class="ri-folder-2-line"></i></div></div>
            ...
            <div class="content-area">
                ...
                <div id="project-view">
                    ...
                    <div id="projectsContent"><div class="text-center py-8"><i class="ri-loader-4-line animate-spin" style="color:var(--green);font-size:1.5rem"></i></div></div>
                </div>
        ...
        <div class="status-bar"><span>© <?php echo date('Y'); ?> CEMS. All rights reserved.</span><span>Teaching Staff Portal &nbsp;•&nbsp; v2.0</span></div>
    </div>
</main>
...
document.querySelectorAll('a.nav-item:not(.locked)').forEach(link=>{
    link.addEventListener('click',e=>{
        if(link.href.includes('teaching_staff.php')&&!link.href.includes('logout')){
            e.preventDefault();
...
function joinActivity(id,name){ _joinType='activity'; _joinId=id; const w=document.getElementById('modalIconWrap'),ic=document.getElementById('modalIcon'); w.style.background='rgba(233,30,140,0.15)'; w.style.border='1px solid rgba(233,30,140,0.3)'; ic.className='ri-calendar-event-line'; ic.style.color='#E91E8C'; document.getElementById('modalTitle').textContent='Join Activity'; document.getElementById('modalName').textContent='"'+name+'"'; const btn=document.getElementById('modalConfirmBtn'); btn.style.background='linear-gradient(135deg,#E91E8C,#8B2FC9)'; btn.innerHTML='<i class="ri-calendar-check-line mr-1"></i>Join Activity'; openJoinModal(); }
function joinProject(id,name){ _joinType='project'; _joinId=id; const w=document.getElementById('modalIconWrap'),ic=document.getElementById('modalIcon'); w.style.background='rgba(29,185,84,0.15)'; w.style.border='1px solid rgba(29,185,84,0.3)'; ic.className='ri-folder-add-line'; ic.style.color='#1DB954'; document.getElementById('modalTitle').textContent='Join Project'; document.getElementById('modalName').textContent='"'+name+'"'; const btn=document.getElementById('modalConfirmBtn'); btn.style.background='linear-gradient(135deg,#1DB954,#16a34a)'; btn.innerHTML='<i class="ri-folder-add-line mr-1"></i>Join Project'; openJoinModal(); }
